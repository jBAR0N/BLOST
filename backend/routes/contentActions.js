...
module.exports = (app)=>{
    ...
    app.post("/like", (req, res)=>{
        if (req.isAuthenticated()) {
            con.query(`SELECT * FROM liked WHERE content_id = ? AND user_id = ?`, [req.body.content, req.user.id], (err, result)=>{
                if (result.length === 0) {
                    ...
                    con.query(`DELETE FROM liked WHERE content_id = ? AND user_id = ?`, [req.body.content, req.user.id], (err)=>{
                        if (err) {res.send({success: false})}
                        else res.send({success: true, action: false})
                    })
                } else res.send({success: false})
